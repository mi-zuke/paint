...
* **`drawing.ts`**: ペンや消しゴムによる実際の描画処理。ストロークのスムージング（手ぶれ補正：指数平滑追尾）アルゴリズム、OKLCHベースのカラー計算、ペンのゆらぎ効果（筆圧風効果）設定、および線分描画における「端点重ね塗りによる黒ずみ・イモムシ現象」を完全に防止する「単一連続パス（Single Continuous Path Rendering）＋ストロークバッファ (`strokeCanvas`) 転写エンジン」を搭載しています。ストローク中は座標リストを連続した 1 本のパス (`beginPath -> moveTo -> lineTo...`) として `strokeCanvas` に一括描画してプレビューし、ペンを離した瞬間 (`commitStrokeToLayer`) に 1 回だけアクティブレイヤーへと合成・転写することで、半透明や等幅ペンにおける端点の重なり濃色化を完全に根絶します。また、手振れ補正 (Stabilize) のアルゴリズムは、PC・iPad 両方で極めて自然な引っかかりのない描画を保証する**純粋なオリジナル追尾ロジック (`positionSmoothing` によるバネ追尾＋ゴム紐半径 `lazyRadius` を超えた分の直接引き寄せ)** を採用しつつ、iPad / Safari / Apple Pencil 環境における過去イベント遅延混入による線分の戻り・暴れを完全に防ぐ**タイムスタンプ逆転排除フィルタ (`lastProcessedTimestamp` 比較判定)** を組み込んでいます。ゴム紐の追尾半径 (`lazyRadius`) は従来の 2/3 倍（最大約 133、初期値 40）へと最適化されており、現在の適用平滑化係数 `getCurrentSmoothingFactor()` を `debug_graph.ts` 上でリアルタイムに確認することができます。
